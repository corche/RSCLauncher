...
class ProcessBuilder {
	...
	build() {
		...
		// ✅ servers.dat overwrite (spawn 직전)
		try {
			// 현재 런처는 cwd/game_directory가 this.gameDir로 고정되어 있으니
			// servers.dat도 this.gameDir에 쓰면 됨.
			// 혹시 구조가 gameDir/.minecraft 인 경우도 대비:
			const candidate = path.join(this.gameDir, ".minecraft");
			const finalDir = fs.existsSync(candidate)
				? candidate
				: this.gameDir;
			...
			fs.copyFileSync(template, path.join(finalDir, "servers.dat"));
		} catch (e) {
			logger.warn("Failed to write servers.dat", e);
		}
